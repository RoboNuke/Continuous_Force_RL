...
class TestHybridActionGMM(unittest.TestCase):
    ...
    def test_log_prob(self):
        ...
        test_case = torch.tensor(
            [[ 0,1,0,1,0,1, 0,0, 0.5,0.5, 1,1, 0,0,0.5,0.5,1,1], # same value
             [ 0,1,0,1,0,1, 0,0, 0,0, 0.5,0.5, 0.5,0.5, 1,1, 0.0,0.0],
             [ 0,1,0,1,0,1, 0.5,0.5, 1,1, 1,1,  1,1, 0,0, 0.5,0.5]]
        )
        ...
        for i in range(3):
            ...
            f = test_case[i,12:18]
            for k in range(6):
                log_probs[i,k] = log( sp[k].item() * probs[p[k].item()][0] + s[k].item() * probs[f[k].item()][1] )
