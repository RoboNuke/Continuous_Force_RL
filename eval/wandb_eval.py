...
class FixedNoiseWrapper(gym.Wrapper):
    ...
    def _wrapped_reset_idx(self, env_ids):
        ...
        # DEBUG: Check noise before override
        print("\n" + "="*80)
        print("DEBUG FixedNoiseWrapper._reset_idx() - Call Parent")

        # Let the base environment complete its reset
        self._original_reset_idx(env_ids)

        # DEBUG: Print noise that was randomly generated by factory_env
        print(f"Random noise from factory_env (env 0): {self.unwrapped.init_fixed_pos_obs_noise[0].cpu().numpy()}")
        print(f"Random noise from factory_env (env 400): {self.unwrapped.init_fixed_pos_obs_noise[400].cpu().numpy()}")

        # DEBUG: Print what we want to apply
        print(f"\nNoise we WANT to apply (env 0):   {self.noise_assignments[0].cpu().numpy()}")
        print(f"Noise we WANT to apply (env 400): {self.noise_assignments[400].cpu().numpy()}")
        ...
        print(f"\nNoise AFTER override (env 0):   {self.unwrapped.init_fixed_pos_obs_noise[0].cpu().numpy()}")
        print(f"Noise AFTER override (env 400): {self.unwrapped.init_fixed_pos_obs_noise[400].cpu().numpy()}")


        print(f"\nInitial fixed pos action frame (env 0):   {self.unwrapped.fixed_pos_action_frame[0].cpu().numpy()}")
        print(f"Initial fixed pos action frame (env 400): {self.unwrapped.fixed_pos_action_frame[400].cpu().numpy()}")
        ...
        print(f"\nFinal fixed pos action frame (env 0):   {self.unwrapped.fixed_pos_action_frame[0].cpu().numpy()}")
        print(f"Final fixed pos action frame (env 400): {self.unwrapped.fixed_pos_action_frame[400].cpu().numpy()}")

        
        print(f"\nInitial actions (env 0):   {self.unwrapped.actions[0, 0:3].cpu().numpy()}")
        print(f"Initial actoins (env 400): {self.unwrapped.actions[400, 0:3].cpu().numpy()}")
        ...
        print(f"\nFinal actions (env 0):   {self.unwrapped.actions[0, 0:3].cpu().numpy()}")
        print(f"Final actions (env 400): {self.unwrapped.actions[400, 0:3].cpu().numpy()}")

        # DEBUG: Verify the actual observation that will be computed
        ...
        print(f"\nExpected fingertip_pos_rel_fixed (env 0): {fingertip_pos_rel_fixed[0].cpu().numpy()}")
        print(f"Expected fingertip_pos_rel_fixed (env 400): {fingertip_pos_rel_fixed[400].cpu().numpy()}")
        print("="*80)
